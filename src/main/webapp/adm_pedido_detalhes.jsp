...
dominio.venda.ItemPedido, dominio.livro.Livro,dominio.livro.Autor,dominio.livro.Categoria, 
...
dominio.venda.StatusPedido, java.time.LocalDate, java.time.format.DateTimeFormatter, dominio.venda.TipoCupom"%>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
            ...
            <div class="row px-5 py-2">
            	...
            	<div class="col-8">
		            ...
		            <div class="row px-5 py-2">
		                <div class="col-12 text-center	">
		                  <input class="form-control" type="text" id="txtIdCliente"
		                      value="Nome do Cliente: <%=pedido.getCliente().getNome()%>" aria-label="Disabled input example" disabled readonly>
		                </div>
		            </div>
    			...
    			<div class="row px-5 py-2">
              	...
              	<div class="row px-5 py-2 text-center">
              		...
              		<%if(pedido.getStatus().getValor() == 1){ %>
              			<form action="/EcomerceLivroLES/adm-pedido-detalhes" method="post">
              				<input type="hidden" name="operacao" value="Alterar"/>
              				<button type="submit" class="btn btn-success" name="txtStatus" value="2">
              					Confirmar Envio do Pedido
              				</button>
              				<p class="text-secondary text-center py-2">
              				*Essa acao ira alterar o status do pedido para "Em Trânsito". Esse processo nao pode ser desfeito.
              				</p>
              			</form>
              		<%}else if(pedido.getStatus().getValor() == 2){ %>
	          			<form action="/EcomerceLivroLES/adm-pedido-detalhes" method="post">
		      				<input type="hidden" name="operacao" value="Alterar"/>
		      				<button type="submit" class="btn btn-success" name="txtStatus" value="3">
		      					Confirmar Entrega do Pedido
		      				</button>
		      				<p class="text-secondary text-center py-2">
		      				*Essa acao ira alterar o status do pedido para "Entregue". Esse processo nao pode ser desfeito.
		      				</p>
		      			</form>
	      			<%} else if(pedido.getStatus().getValor() == 4 ||
	      						pedido.getStatus().getValor() == 5 ||
	      						pedido.getStatus().getValor() == 6 ||
	      						pedido.getStatus().getValor() == 7) { %>
	      			
              		<form action="/EcomerceLivroLES/adm-pedido-troca-detalhes" method="post">
              			<input type="hidden" name="idPedido" value="<%=pedido.getId()%>"/>
              			
              			<button type="submit" class="btn btn-primary" name="operacao" value="ConsultarPorPedido">
		      					Vizualizar Pedido de Troca
		      			</button>
                   ...
                   <div class="row px-5 py-2 text-center">
			            <form action="/EcomerceLivroLES/adm-pedidos" method="post">
